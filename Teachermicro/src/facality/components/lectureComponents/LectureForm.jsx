...
import { AcademicAPI, WorkAPI } from "../../api/apis";
...
export default function LectureForm({ onSuccess, refreshList }) {
  ...
  const fetchSubjects = useCallback(async () => {
    ...
    try {
      const { data } = await AcademicAPI.get(`/subjects?department=${link.course}${link.semester ? `&semester=${link.semester}` : ""}`);
      setSubjects(data?.subjects || []);
    } catch (err) {
      console.error("Subject fetch failed", err);
    }
  }, [link.course, link.semester]);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      setLink({ topic: "", subject: "", linkas: "", department: "", semester: "", section: "" });
      refreshList?.();
      onSuccess?.();
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8 relative z-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              <div className="space-y-1.5 sm:col-span-2 md:col-span-3">
                  ...
                  <div className="relative group/input">
                      ...
                      <input 
                        ...
                        className="w-full pl-14 pr-5 py-3.5 bg-white  rounded-[10px] text-[11px] font-black uppercase outline-none focus:ring-2 focus:ring-indigo-900 transition-all shadow-sm" 
                        required 
                      />
                  </div>
              </div>
              ...
              <div className="space-y-1.5 sm:col-span-2 md:col-span-3">
                  ...
                  <div className="relative group/input">
                      ...
                      <select 
                        ...
                        value={link.subject} 
                        onChange={handleChange} 
                        className="w-full pl-14 pr-4 py-3.5 bg-white  rounded-[10px] text-[10px] font-black uppercase appearance-none outline-none focus:ring-2 focus:ring-indigo-900 transition-all cursor-pointer shadow-sm" 
                        required
                      >
              ...
              <div className="space-y-1.5">
                  ...
                  <div className="relative">
                      ...
                      <select name="course" value={link.course} onChange={handleChange} className="w-full pl-14 pr-4 py-3 bg-white  rounded-[10px] text-[10px] font-black uppercase appearance-none outline-none focus:ring-2 focus:ring-indigo-900 transition-all cursor-pointer shadow-sm" required>
              ...
              <div className="space-y-1.5">
                  ...
                  <div className="relative">
                      <FiBookOpen className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" />
                      <select name="semester" value={link.semester} onChange={handleChange} className="w-full pl-14 pr-4 py-3 bg-white  rounded-[10px] text-[10px] font-black uppercase appearance-none outline-none focus:ring-2 focus:ring-indigo-900 transition-all cursor-pointer shadow-sm" required>
              ...
              <div className="space-y-1.5">
                  ...
                  <div className="relative">
                      ...
                      <select name="section" value={link.section} onChange={handleChange} className="w-full pl-14 pr-4 py-3 bg-white  rounded-[10px] text-[10px] font-black uppercase appearance-none outline-none focus:ring-2 focus:ring-indigo-900 transition-all cursor-pointer shadow-sm" required>
              ...
              <div className="space-y-1.5 sm:col-span-2 md:col-span-3">
                  ...
                  <div className="relative group/input">
                      ...
                      <input 
                        ...
                        className="w-full pl-14 pr-5 py-3 bg-white  rounded-[10px] text-[10px] font-black uppercase outline-none focus:ring-2 focus:ring-indigo-900 transition-all shadow-sm" 
                        required 
                      />
                  </div>
              </div>
          ...
          <div className="flex justify-end pt-4 border-t border-slate-50">
              <button 
                  ...
                  className="px-12 py-3.5 bg-slate-900 border border-slate-800 text-white rounded-[10px] text-[10px] font-black uppercase tracking-[0.3em] italic shadow-xl hover:bg-indigo-600 hover:shadow-indigo-100 transition-all flex items-center justify-center gap-3 w-full md:w-fit"
              >
                  <FiMonitor size={14} />
